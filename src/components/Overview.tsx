    ...
    <div className="space-y-8 animate-fade-in text-white">
      ...
      <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-4 border-b border-white/8 pb-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-white/5 border border-white/10 py-2 px-3.5 rounded-lg">
            <span className="text-[10px] text-[#C5A880] font-extrabold uppercase tracking-widest">Active Node:</span>
            <select
              id="node-selector-dropdown"
              value={activeBank.address}
              onChange={(e) => onBankChange(e.target.value)}
              className="bg-transparent text-xs text-white font-bold focus:outline-none cursor-pointer pr-1"
            >
              {allBanks.map((bank) => (
                <option key={bank.address} value={bank.address} className="bg-[#141416] text-white">
                  {bank.name}
                </option>
              ))}
            </select>
